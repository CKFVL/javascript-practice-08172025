...
getOrders(1).then(d => console.log(d)).catch(err => console.log(err));
//getOrders(11).then(d=>console.log(d)).catch(err=>console.log(err));

function getOrdersAsync(userId) {
  return new Promise((resolve, reject) => {
    if (userId === 1) {
      resolve(setTimeout(() => {
        return { id: 1, name: 'pavan' };
      }, 2000))
    } else {
      reject(new Error('user not found'))
    }
...
getOrdersAsync(1).then(d => console.log(d)).catch(err => console.log(err));
getOrdersAsync(11).then(d => console.log(d)).catch(err => console.log(err));

What's wrong with getOrdersAsync()?
setTimeout() does NOT return the value from its callback and its result is a timer ID.
  So, The Promise is therefore resolved immediately, rather than after 2 seconds.

Correct way:
Call resolve() inside the setTimeout callback:

function getOrdersAsync(userId) {
  return new Promise((resolve, reject) => {
    if (userId === 1) {
      setTimeout(() => {
        // Think of it as: "2 seconds have passed"
        resolve({ id: 1, name: 'pavan' }); // fulfill the Promise now
      }, 2000);
    } else {
      reject(new Error('user not found'));
    }
